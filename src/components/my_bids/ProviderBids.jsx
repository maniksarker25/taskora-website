  ...
  const handleDeleteBid = async (bidId) => {
    ...
    try {
      ...
      if (error?.status === 'FETCH_ERROR' || error?.status === 'PARSING_ERROR') {
        ...
        toast.error(error?.data?.message || 'Failed to delete bid. Please try again.', {
          style: {
            backgroundColor: '#fee2e2',
            color: '#991b1b',
            borderLeft: '6px solid #ef4444',
    ...
    <div>
      <div className="flex flex-col lg:flex-row lg:justify-between gap-8 border rounded-2xl p-6 bg-white shadow mt-8 ">
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-lg font-semibold text-black">To Be Done On</p>
          ...
          {/* <div className="flex-1 min-w-0">
            <div className="w-full">
              ...
              <p className="text-gray-700 leading-relaxed whitespace-pre-wrap break-all overflow-hidden w-full">
      ...
      <div className="mt-4 space-y-4">
        {activeTab === 'bids' && info?.length > 0 ? (
          info.map((bid) => (
            <div key={bid._id} className="flex flex-col lg:flex-row gap-4 p-4 border rounded-lg">
              <div className="flex flex-col justify-between gap-10 bg-[#E6F4F1] rounded-xl px-4 py-4 md:max-h-[200px] min-h-[150px]">
                ...
                {(user?.profileId === bid?.provider?.profileId ||
                  user?.profileId === bid?.provider?._id) && (
        ...
        {activeTab === 'questions' && (
          <div className="bg-white p-4 sm:p-6 rounded-xl border border-gray-100 shadow-sm mb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4 md:text-center sm:text-left">
              Ask a Question
            </h3>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-medium text-gray-900">{q?.provider?.name || 'Anonymous'}</h3>
                  <span className="text-xs text-gray-400">{formatTimeAgo(q.createdAt)}</span>
                </div>
                ...
                {q.answer && (
                  <div className="bg-green-50 p-3 rounded-lg border border-green-100">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {q.answeredAt && (
                        <span className="text-xs text-gray-500">{formatTimeAgo(q.answeredAt)}</span>
                      )}
                    </div>
                    <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">
                      {q.answer}
