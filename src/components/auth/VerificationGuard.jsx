...
const VerificationGuard = ({ children }) => {
  const { isAuthenticated, isAuthLoading, isProfileLoading, profileData, user } = useAuth();
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user?.role !== "admin") {
      ...
      if (user?.role === "provider") {
        if (!data.isBankVerificationNumberApproved) {
        ...
        if (!data.isIdentificationDocumentApproved) {
